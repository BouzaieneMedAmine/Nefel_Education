...
public class Test {
    public static void main(String[] args) {
        ...
        System.out.printf("Physician pin assign test 1 (too short): %s\n", pinTest1);

        String pinTest2 = doctor10.assignPin(4000) ? "PASS" : "FAIL";
        System.out.printf("Physician pin assign test 2 (valid): %s\n", pinTest2);

        String authTest1 = doctor10.accessAuthorized(1234) ? "FAIL" : "PASS";
        System.out.printf("Physician auth test 1 (wrong ID): %s\n", authTest1);

        String authTest2 = doctor10.accessAuthorized(1) ? "PASS" : "FAIL";
        System.out.printf("Physician auth test 2 (correct ID): %s\n", authTest2);
        ...
        System.out.printf("Admin pin assign test 1 (too short): %s\n", pinTest3);

        String pinTest4 = roseTyler.assignPin(424242) ? "PASS" : "FAIL";
        System.out.printf("Admin pin assign test 2 (valid): %s\n", pinTest4);

        String authTest3 = roseTyler.accessAuthorized(1234) ? "FAIL" : "PASS";
        System.out.printf("Admin auth test 1 (wrong ID): %s\n", authTest3);

        String authTest4 = roseTyler.accessAuthorized(1234) ? "FAIL" : "PASS";
        System.out.printf("Admin auth test 2 (wrong ID again): %s\n", authTest4);

        String authTest5 = roseTyler.accessAuthorized(2) ? "PASS" : "FAIL";
        System.out.printf("Admin auth test 3 (correct ID): %s\n", authTest5);

        // Test multiple failed access attempts for donnaNoble
        ...
        // Print incidents using default method from interface
        System.out.println("\nSecurity Incidents for Admin (donnaNoble):");
